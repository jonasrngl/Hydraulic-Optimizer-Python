...
def SingleExecution(seed, populationSize, mutationRate, mutation, crossoverRate, crossover, currentAlgorithm, runner, ref_dirs, seedRound):
    ...
    for i in range(2 * Globals.numberOfThreads):
        Globals.dockers[Globals.availablePorts[i]] = Globals.client.containers.run(
            "epanet-docker", "app/Main.py", mem_limit="128m", network_mode="host", environment=[f"PORT={Globals.availablePorts[i]}"], detach=True)
    ...
    algorithm = AlgorithmSelection.SelectAlgorithm(name=currentAlgorithm, pop_size=populationSize, samp=Globals.sampling,
                                                   co=crossover, mt=mutation, no=Globals.n_objectives, nd=Globals.n_dimensions, rd=ref_dirs)

    currentAlgoStart = time.time()

    res = minimize(problem, algorithm, Globals.stop_criteria,
    ...
    currentWriter = csv.writer(
        open(f"results/{currentAlgorithm}.csv", 'a', encoding="utf-8"))
    ...
    labelsRow = np.concatenate(
        (labelsRow, "Overall quality(success rate)"), axis=None)

    if (counter == True):
        labelsRow = np.concatenate(
            (labelsRow, "Objective Function Calls"), axis=None)
    ...
    valuesRow = np.concatenate(
        (valuesRow, str(Globals.generations)), axis=None)
    ...
    valuesRow = np.concatenate(
        (valuesRow, str(problem.overallSuccesses)), axis=None)
    valuesRow = np.concatenate(
        (valuesRow, str(successes/len(res.F))), axis=None)
    valuesRow = np.concatenate(
        (valuesRow, str((problem.overallSuccesses)/(problem.counter))), axis=None)
    if (counter == True):
        valuesRow = np.concatenate(
            (valuesRow, str(problem.counter)), axis=None)
        valuesRow = np.concatenate(
            (valuesRow, str((successes/len(res.F))/(problem.counter))), axis=None)
    currentEndMin = currentAlgoEnd // 60
    valuesRow = np.concatenate((valuesRow, str(currentEndMin)), axis=None)
    valuesRow = np.concatenate(
        (valuesRow, str(currentAlgoEnd - (currentEndMin * 60))), axis=None)
...
def ExecuteAlgorithms(**kwargs):
    ...
    ref_dirs = get_reference_directions(
        "das-dennis", Globals.n_dimensions, n_partitions=populationSize - 1, seed=seed)
    ...
    if (allOfThem):
        for currentAlgorithm in AlgorithmSelection.REF:
            if (Globals.saveState):
                ...
                SingleExecution(seed, populationSize, mutationRate, mutation, crossoverRate,
                                crossover, currentAlgorithm, runner, ref_dirs, seedRound)
...
if __name__ == '__main__':
    ...
    Globals.client.images.build(
        path="./EpanetDocker/", tag="epanet-docker", rm=True, nocache=False)
    ...
    Loops.SeedLoop(Loops.PopulationLoop, Loops.MutationRateLoop,
                   Loops.CrossoverRateLoop, ExecuteAlgorithms)
